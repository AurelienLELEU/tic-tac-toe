...
labeluti2 = tk.Label(start, text="Joueur 1 ?")
entryuti2 = tk.Entry(start, textvariable=joueur1)
labelpass2 = tk.Label(start, text="Joueur 2 ?")
...
def fwin(x, y):
    ...
    if i == 9:
        labelnul = tk.Label(fin, text="Match nul")
        labelnul.grid()
        restart()
    elif grille[x, y] == 1:
        labelwin = tk.Label(fin, text=j2+" a gagné")
        labelwin.grid()
        restart()
        scorej2 += 1
    else:
        labelwin = tk.Label(fin, text=j1+" a gagné")
        labelwin.grid()
        restart()
        scorej1 += 1
...
def restart():
    buttonrestart = tk.Button(fin, text="On Recommence?", command=btres)
    buttonrestart.grid()
